...
class McpHttpServer(private val mcp: McpServer, private val auth: DesignerAuth) {
    ...
    /**
     * True when the endpoint is reachable only from this machine. Backported from the 8.3 line so
     * the startup warnings can distinguish the case where no credential is required from the case
     * where one is.
     */
    ...
    /**
     * Backported with the fixed [DEFAULT_PORT]: without this a second Designer on one machine dies
     * with a bare BindException and no endpoint at all, which a fixed default would make routine.
     */
     ...
     * structured error, and carrying the same realm the gateway's bearer control uses.
     */
    ...
    companion object {
         ...
         * surely as the old per-session secret did. `-Dmcp.designer.port=0` restores OS assignment.
         */
