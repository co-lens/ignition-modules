 ...
 * `ConfigurationManager` each extend [ProjectManager] — so one code path serves both.
 *
 ...
 * collections is not: 8.3 has the generic `getNames()`/`find(name)` on [ProjectManager],
...
class ResourceScanner(private val context: GatewayContext) {
    ...
    private fun projectManager(): ProjectManager = context.projectManager

    /**
     * Gateway config — 8.3 only. On this line `GatewayContext` has no `getConfigurationManager()`
     * at all: 8.1 keeps gateway config in `config.idb` rather than on disk, so there is nothing a
     * scan could read. Always null here; the tool reports `config` as unavailable rather than
     * failing, which is what keeps the tool surface identical to the 8.3 line.
     */
    private fun configManager(): ProjectManager? = null
     ...
     *
     * **This is the 8.1 form.** 8.3 uses the generic `manager.names` / `manager.find(name)` on
     * `ResourceCollectionManager`, which also serves its config collection; 8.1 has only projects.
     *
     ...
     */
    private fun snapshot(manager: ProjectManager): Map<String, Map<String, String>> =
        manager.projectNames.associateWith { name ->
            runCatching {
                manager.getProject(name).orElse(null)
                    ?.allResources
                    ?.entries
