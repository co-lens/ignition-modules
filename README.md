...
Kotlin module. 49 tools: the 8.3 line's 56, less seven not yet ported. The other difference that
...
| Tool count | 56 | 49 — the seven below are absent |
...
**Seven tools on the 8.3 line are absent here**, all added to `main` after this branch forked:
...
The JVM performance tools — `jvm_health`, `thread_dump`, `thread_hotspots` — were the first wave of
that port and are present on this branch.

None of the remaining seven is blocked by the 8.1 platform — every SDK API they need exists in
...
For the 49 tools that *are* here, the names, arguments and behaviour are identical to the 8.3 line,
