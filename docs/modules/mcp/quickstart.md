...
```bash
gh release download mcp-v0.5.0 --repo co-lens/ignition-modules --pattern 'Ignition-MCP-0.5.0.modl*'
sha256sum -c Ignition-MCP-0.5.0.modl.sha256
```
...
```bash
gh release download mcp81-v0.4.0 --repo co-lens/ignition-modules --pattern 'Ignition-MCP-81-0.4.0.modl*'
sha256sum -c Ignition-MCP-81-0.4.0.modl.sha256
```
...
```json
{"status":"ok","server":"ignition-mcp","version":"0.5.0","mcpEndpoint":"/data/mcp/mcp",
